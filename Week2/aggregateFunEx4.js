...
const GET_RESEARCH_PAPERS_AND_NUMBER_OF_AUTHORS = `
    SELECT research_papers.paper_title, count(research_papers_authors.author_id) 
    AS 'Number of Authors By Each Paper' 
...
const GET_RESEARCH_PAPERS_PUBLISHED_BY_FEMALES = `
    SELECT gender, COUNT(research_papers_authors.paper_id) AS sum_of_published_papers
    FROM authors 
    JOIN research_papers_authors 
    ON research_papers_authors.author_id = authors.author_id 
    GROUP BY gender 
    HAVING gender = 'f';`;
